			...
			if (SubChannels <= SubChannelsClients) {
				...
				for (c = 0; c <= SubChannels; c++) {
					if (ChannelPrefixArray == 0) {
						var ChanNum = c+1;
						var ChanName = ChannelPrefix + ' ' + ChanNum;
					}
				...
				if (ChannelCreateCheck == 1) {
					var create = backend.createChannel({ name: ChannelNameFinish, parent: GroupChannel, permanent: true, codec: parseInt(ChannelCodec), codecQuality: parseInt(parseInt(ChannelQuality) + 1), maxClients: parseInt(ChannelMaxClients), description: ChannelDescription});//, neededJoinPower: parseInt(ChannelJoinPower), neededTalkPower: parseInt(ChannelTalkPower)});
