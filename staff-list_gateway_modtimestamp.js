...
registerPlugin(
    {
        name: 'Staff List - Gateways (timed)',
        version: '1.10.4',
        ...
        vars: [
            ...
            {
                name: 'afkChannels',
                title: 'AFK-Channels > Define a list of channel IDs that should NOT count as AFK-channels! (*)',
                type: 'strings',
                indent: 2,
                conditions: [
                    {
    ...
    (_, scriptConfig, meta) => {
        ...
        function isInAfkChannel(client) {
            for (let channel of client.getChannels()) {
                if (config.afkChannels.includes(channel.id())) return false;
            }
        ...
        function main() {
            ...
            backend.getClients().forEach(client => {
                ...
                var time = new Date();
                var timestamp = time.getTime();
                var inChannel = client.getAudioChannel();
                var inChannelId = inChannel.id();
                // engine.log('Client: ' + client.name() + ' ist im Channel' + inChannelId);
                if (clientStaffGroups.length && inChannelId != '528' ) { // Wenn Client im geheimen Raum ist, nicht darstellen!
            ...
            event.on('clientMove', event => {
                ...
                if (toChannel !== undefined) var toChannelId = toChannel.id();

                // engine.log ('toChannel: ' + toChannelId);

                // check if it's a relevant client & not in secret channel
                if (groups.length && toChannelId != '528') {
                    var time = new Date();
                    var timestamp = time.getTime();
